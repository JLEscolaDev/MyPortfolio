...
func routes(_ app: Application) throws {
    ...
    let projects: [Project] = [
        ...
        Project(
            name: "SwiftUI Offline Bingo — macOS",
            description: "Bingo offline con fondos navideños generados por IA, voces EN/ES, rimas y música. Ajustes para volumen, rimas y velocidad de lectura.",
            image: "/Images/SwiftUIOfflineBingo/BingoPreview.jpg",
            link: "swiftUIOfflineBingo"
        ),
    ...
    // NUEVA ruta: SwiftUI Offline Bingo
    app.get("swiftUIOfflineBingo") { req in
        let language = req.selectedLanguage
        return req.view.render("swiftUIOfflineBingo_\(language)")
    }
